...
``` bash
curl -s https://raw.githubusercontent.com/mrjdomingus/odoo-17-docker-compose/master/run.sh | bash -s odoo-one 10019 20019
```
and/or run the following to set up another Odoo instance @ `localhost:11019` (default master password: `mag1ster`):

``` bash
curl -s https://raw.githubusercontent.com/mrjdomingus/odoo-17-docker-compose/master/run.sh | bash -s odoo-two 11019 21019
```
